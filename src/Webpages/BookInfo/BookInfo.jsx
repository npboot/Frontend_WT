...
//   const [author, setAuthor] = useState("John Doe");
//   const [publishDate, setPublishDate] = useState("10-12-2023");
//   const [summary, setSummary] = useState("the brown fox jumps over the lazy dog...");
//   const [books, setBooks] = useState([]);
//   const [isbn, setISBN] = useState("444445");
//   const location = useLocation();
//   const navigate = useNavigate();
...
//   const [expandedBookIds, setExpandedBookIds] = useState(new Set());
//   const [copyHistory, setCopyHistory] = useState({}); // State to store history for each copy

  // const toggleExpand = async (copyId) => {
  //   setExpandedBookIds(prev => {
  //     const updated = new Set(prev);
  //     if (updated.has(copyId)) {
  //       updated.delete(copyId);
  //     } else {
  //       updated.add(copyId);
  //       // Fetch copy history when expanded
  //       fetchCopyHistory(copyId);
  //     }
  //     return updated;
  //   });
  // };
...
//       const allCopyInfoByISBN = `${baseBookInfoRequestURL}?isbn=${state?.isbn}`;
//       try {
//         const response = await fetch(allCopyInfoByISBN);
//         if (!response.ok) {
//           throw new Error(`HTTP error! status: ${response.status}`);
//         }
//         const allCopyInfoByISBNJSON = await response.json();
//         updateBookInformation(allCopyInfoByISBNJSON);
//       } catch (error) {
//         console.error('Error fetching catalog data:', error);
//       }
